   ...
   localStorage.setItem('emailSent', false);
      ...
      { // Check if email has not been sent yet
        ...
        const fetchData = async () => {
            try {
                ...
                const venueResponse = await axios.get(`http://13.53.132.117:8080/venues/${bookingDetails.booking.venueId}`);
                setVenueName(venueResponse.data.venue_name);
                localStorage.setItem('venueName', venueResponse.data.venue_name);
                localStorage.setItem('movieName', bookingDetails.booking.movieName);

                const movieResponse = await axios.get(`http://13.53.132.117:8080/get-movies/${bookingDetails.booking.movieName}`);
    ...
    useEffect(() => {
        ...
        if (venueName && posterURL) {
            console.log("Hi 1")
          console.log(localStorage.getItem('emailSent'))
