...
const MovieDetailsPage = ({ bodyClassName }) => {
    ...
    useEffect(() => {
        const fetchMovieDetails = async () => {
            try {
                const response = await axios.get(`http://13.53.132.117:8080/movies/${id}`);
    ...
    const fetchOpenedMovie = async (selectedDate) => {
        try {
            const response = await axios.get(`http://13.53.132.117:8080/opened_movies?movie_id=${id}&date=${selectedDate}`);
        ...
        const fetchVenueDetails = async () => {
            if (openedMovie && openedMovie.length > 0 && openedMovie[0]?.venues) {
                const venuePromises = openedMovie[0].venues.map(venueId =>
                    axios.get(`http://13.53.132.117:8080/venues/${venueId}`)
                );
